...
def load_data(data_file, class_file, char):
    if char:
        ...
        data_examples = list(x.strip() for x in open(data_file, "r", encoding='utf-8').readlines())
        print("Data Read & Transformed to Word Finished!")

    class_examples = list(x.strip() for x in open(class_file, "r", encoding='utf-8').readlines())
